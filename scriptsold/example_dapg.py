...
if __name__ == "__main__":
    ...
    cfg = parse_cfg(
        default_cfg_path=osp.join(
            osp.dirname(__file__), "sample_configs/default_dapg.yml"
        )
    )
    env_id = cfg["env_id"]
    num_cpu = cfg["cpu"]
    ...
    demo_trajectories["observations"] = torch.as_tensor(
        np.vstack(demo_trajectories["observations"]), dtype=torch.float32
    )
    demo_trajectories["actions"] = torch.as_tensor(
        np.vstack(demo_trajectories["actions"]), dtype=torch.float32
    )

    def demo_trajectory_sampler(batch_size):
        transition_inds = np.random.randint(
            0, len(demo_trajectories["observations"]), size=batch_size
        )
        return dict(
