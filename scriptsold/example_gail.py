...
if __name__ == "__main__":
    ...
    def expert_reward(rewards, observations, actions):
        expert_rew_proportion = 1.0  # gail_hparam_args["expert_rew_proportion"]
        with torch.no_grad():
            ...
            rews = expert_rews * expert_rew_proportion + rewards * (
                1 - expert_rew_proportion
            )
            return rews

    expert_trajectories = np.load(
        osp.join(osp.dirname(__file__), "expert_cartpole.npy"), allow_pickle=True
    )
