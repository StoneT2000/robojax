"""
Temporary RecordEpisode wrapper copied from ManiSkill2 adapted for gymnasium until ManiSkill2 upgrades
"""
import copy
import time
            ...
            for k, v in obs.items():
                if "rgb" in k and v.ndim == 4:
                    ...
                    if not np.all(np.logical_and(v >= 0, v < 2**6)):
                        raise RuntimeError("The depth map({}) is invalid with min({}) and max({}).".format(k, v.min(), v.max()))
