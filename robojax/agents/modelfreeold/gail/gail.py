...
class GAIL:
    ...
    def train(
        ...
        for epoch in range(start_epoch, n_epochs + start_epoch):
            ...
            print(
                f"===rollout out done ({rollout_delta_time}s), collected {len(rollout_obs)} steps==="
            )
            ...
            for batch_idx in range(disc_iters):
                ...
                batch_slice = slice(
                    max(0, (batch_idx) * disc_mini_batch_size),
                    (batch_idx + 1) * disc_mini_batch_size,
                )

                b_rollout_obs = rollout_obs[batch_slice]
                b_rollout_act = rollout_act[batch_slice]
                b_rollout_obs = (
                    torch.as_tensor(b_rollout_obs, device=device, dtype=torch.float32)
                    if obs_to_tensor is None
                    else obs_to_tensor(b_rollout_obs)
                )
                b_rollout_act = (
                    torch.as_tensor(b_rollout_act, device=device, dtype=torch.float32)
                    if act_to_tensor is None
                    else act_to_tensor(b_rollout_obs)
                )
                ...
                discrim_loss = discriminator_criterion(
                    g_o, torch.ones((len(g_o), 1), device=self.device)
                ) + discriminator_criterion(
                    e_o, torch.zeros((len(e_o), 1), device=self.device)
                )  # expert
            ...
            logger.store(
                "train",
                DiscriminatorUpdateTime=(disc_update_end_time - disc_update_start_time)
                * 1e-9,
                append=False,
            )
            ...
            logger.store(
                "train",
                PPOUpdateTime=(ppo_update_end_time - ppo_update_start_time) * 1e-9,
                append=False,
            )
