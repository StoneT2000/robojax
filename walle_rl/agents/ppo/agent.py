...
from walle_rl.agents.ppo.buffer import Batch, PPOBuffer
...
class PPO(Policy):
    ...
    def train_loop(
        ...
        for epoch in range(start_epoch, start_epoch + n_epochs):
            ...
            logger.store(
                "train",
                env_interactions=steps_per_epoch * buffer.n_envs * (epoch + 1),
                append=False,
            )
    ...
    def train_step(
        ...
        for update_iter in range(update_iters):
            batch = buffer.sample_batch(
                key=next(rng), batch_size=batch_size, drop_last_batch=True
            )
            ...
            logger.store("train", critic_loss=info_c["critic_loss"])
            if info_a is not None:
                logger.store(
                    "train", actor_loss=info_a["loss_pi"], entropy=info_a["entropy"]
                )

        update_end_time = time.time_ns()
        logger.store(
            "train",
            update_time=(update_end_time - update_start_time) * 1e-9,
            append=False,
        )
    ...
    @staticmethod
    @functools.partial(
        jax.jit, static_argnames=["clip_ratio", "update_actor", "update_critic"]
    )
    def update_parameters_step(
        ...
        if update_actor:
            grads_a_fn = jax.grad(
                PPO.actor_loss_fn(
                    clip_ratio=clip_ratio, actor_apply_fn=actor.apply_fn, batch=batch
                ),
                has_aux=True,
            )
        ...
        if update_critic:
            grads_c_fn = jax.grad(
                PPO.critic_loss_fn(critic_apply_fn=critic.apply_fn, batch=batch),
                has_aux=True,
            )
        ...
        return dict(
            new_actor=new_actor, new_critic=new_critic, info_a=info_a, info_c=info_c
        )
